...
class DiffusionDecoder(nn.Module):
    def __init__(self,
        ...
        self.unet = unet.Unet(dim=unet_channels, out_dim=unet_out_channels, dim_mults=dim_mults,
                              groups=groups, channels=unet_in_channels, with_time_emb=with_time_emb)

    def marginal_prob(self, mu, x, t):
        log_mean_coeff = -0.25 * t ** 2 * \
            (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
        mean = torch.exp(log_mean_coeff[:, None, None]) * \
            x + (1-torch.exp(log_mean_coeff[:, None, None])) * mu
    ...
    def cal_loss(self, x, mu, t, z, std, g=None):
        ...
        loss = torch.square(
            grad + z / std[:, None, None]) * torch.square(std[:, None, None])
        return loss

    def forward(self, mu, y=None, g=None, gen=False):
        if not gen:
            t = torch.FloatTensor(y.shape[0]).uniform_(
                0, self.T-self.delta_t).to(y.device)+self.delta_t  # sample a random t
            ...
            with torch.no_grad():
                ...
                for n in tqdm(range(self.N - 1, 0, -1)):
                    ...
                    grad = self.unet(x, t)
                    y_t = y_t_plus_one-0.5*self.delta_t * \
                        self.discrete_betas[n]*(mu-y_t_plus_one-grad)
                    y_t_plus_one = y_t
...
class DiffusionGenerator(nn.Module):
    ...
    def forward(self, x, x_lengths, mel_target, mel_lengths=None, y=None, y_lengths=None, g=None, gen=False, noise_scale=1., length_scale=1.):
        ...
        if gen:
            ...
            z_logs = torch.matmul(attn.squeeze(1).transpose(
            ...
            with torch.no_grad():
                ...
                logp1 = torch.sum(-0.5 * math.log(2 * math.pi) -
                                  x_logs, [1]).unsqueeze(-1)  # [b, t, 1]
                # [b, t, d] x [b, d, t'] = [b, t, t']
